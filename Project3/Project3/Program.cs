...
namespace Project3
{
    class Program
    {
        ...
        static List<ReturnData> IntersectionPoints (double pixelX, double pixelY)
        {
            List<ReturnData> intersectionPoints = new List<ReturnData>();
            foreach (Shape shape in _shapes)
            {
                ReturnData pointColor = shape.Intersection(_camera.CameraPoint, _camera.GetRay(pixelX, pixelY));
                if (pointColor != null)
        ...
        static Color Illuminate(ReturnData returnData)
        {
            ...
            foreach (Light light in _lights)
                color += light.ShineOnShape(returnData);
            ...
            for (int i = xStart; i < xStart + xWidth; i++)
            {
                for (int j = yStart; j < yStart + yWidth; j++)
                {
                    ...
                    for (int k = 0; k < _antiAliasX; k++)
                    {
                        ...
                        {
                            List<ReturnData> intersectionPoints = IntersectionPoints(i + randomX * k, j + randomY * n);
